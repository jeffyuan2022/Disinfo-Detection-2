...
class SensationalismFeatureGenerator:
    ...
    def transform(self, X):
        ...
        # 2. Average word length: Longer words might indicate formal writing, while shorter words could imply sensationalism
        X['avg_word_length'] = X['statement'].apply(lambda x: np.mean([len(word) for word in x.split()]))

        # 3. Lexical diversity: Ratio of unique words to total words (lower diversity could mean repetitive sensational words)
        X['lexical_diversity'] = X['statement'].apply(lambda x: len(set(x.split())) / (len(x.split()) + 1))

        # 4. Sentiment polarity and subjectivity: Sensational text tends to be more emotionally charged and subjective
        ...
        # 5. Readability metrics (Flesch reading ease score): Sensational articles might have lower readability scores
        def flesch_reading_ease(text):
            sentence_count = max(len(re.split(r'[.!?]+', text)), 1)
            word_count = len(text.split())
            syllable_count = sum([self.syllable_count(word) for word in text.split()])
            if word_count > 0:
        ...
        numerical_features = X[['exclamation_count', 'question_mark_count', 'all_caps_count',
                                'avg_word_length', 'lexical_diversity', 'sentiment_polarity',
